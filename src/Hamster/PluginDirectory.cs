...
namespace Hamster
{
    ...
    {
        private readonly Dictionary<string, IPlugin> items;
        private readonly IEventTrigger trigger;
        ...
        public void Register(string name, IPlugin plugin)
        {
            lock (items)
            {
                ...
                {
                    throw new ArgumentException("There is already a plugin registered with the name '" + name + "'.", nameof(name));
                }
